<?php
	...
	function delete_master_call()
	{ 
		include "lib/config.php";					
		$query = "delete from cbc_master_call where cbc_status_call = '0'";		
	...
	function update_master_call_sudah_confirm($ID_MASTER_CALL,$KONFIRMASI,$TGL_NYALA)
	{
		include "lib/config.php";		
		$query = "UPDATE cbc_master_call SET INFORMASI_07='$KONFIRMASI',INFORMASI_08='$TGL_NYALA' where ID_MASTER_CALL = $ID_MASTER_CALL";		
		$stmt = oci_parse($connection, $query);						
		echo "alert($query)";
		if(oci_execute($stmt))
		{ $result = 1;}
	...
	function update_master_call_flag_gagal($ID_MASTER_CALL,$FLAG_GAGAL,$ALASAN_GAGAL,$KETERANGAN)
	{ 
 		include "lib/config.php";		
		$query = "UPDATE cbc_master_call SET INFORMASI_01='$KETERANGAN',INFORMASI_02='$FLAG_GAGAL',INFORMASI_03='$ALASAN_GAGAL' where ID_MASTER_CALL = $ID_MASTER_CALL";		
		$stmt = oci_parse($connection, $query);						
		if(oci_execute($stmt))
		{ $result = 1;}
		else 
	...
	function update_master_call_gagal_callback($ID_MASTER_CALL,$ID_USER,$TGL_GAGAL)
	{ 
 		include "lib/config.php";		
		$query = "UPDATE cbc_master_call SET cbc_status_call='3',informasi_05=$ID_USER,informasi_06=to_date('$TGL_GAGAL','YYYY-MM-DD') where ID_MASTER_CALL = $ID_MASTER_CALL";		
		$stmt = oci_parse($connection, $query);						
		if(oci_execute($stmt))
		{ $result = 1;}
		else 
	...
	function gagal_callback($var_JENIS_MUTASI) 
	{	
		include "lib/config.php";		
		$query = "select * from CBC_MASTER_CALL where CBC_JENIS_MUTASI = $var_JENIS_MUTASI AND CBC_STATUS_CALL='3' ";
		$statement	= oci_parse($connection, $query);
		oci_execute($statement);
		oci_fetch_array ($statement, OCI_ASSOC);
		$row_count	= oci_num_rows($statement);	
	...
	//fungsi belum callback editan taufik
	function belum_callback_taufik($var_JENIS_MUTASI) 
	{	
		include "lib/config.php";		
		$query = "select * from 
					CBC_MASTER_CALL 
					where  CBC_STATUS_CALL='0' ";
		$statement	= oci_parse($connection, $query);
		oci_execute($statement);
		oci_fetch_array ($statement, OCI_ASSOC);
		$row_count	= oci_num_rows($statement);	
	...
	function cbc_master_call_search_by_NO_REGISTRASI($var_NO_REGISTRASI)
	{	
		include "lib/config.php";		
		$query = "select * from CBC_MASTER_CALL where NO_REGISTRASI = $var_NO_REGISTRASI";
		$statement	= oci_parse($connection, $query);
		oci_execute($statement);
		oci_fetch_array ($statement, OCI_ASSOC);
		$row_count	= oci_num_rows($statement);	
